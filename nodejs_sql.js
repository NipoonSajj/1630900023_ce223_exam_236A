...
const pool = mysql.createPool({
    ...
    user : 'root',
    password : '',
    database : 'nodejs_lottery' //Connect Database from info.sql (Import to phpMyAdmin)
})
...
app.get('/:id',(req, res) => {
    ...
    pool.getConnection((err, connection) =>{ //err คือ connect ไม่ได้ or connection คือ connect ได้บรรทัดที่ 13-19
        ...
        connection.query('SELECT * FROM info WHERE `id` = ?', req.params.id, (err, rows) => {
            connection.release();
            if(!err){ 
                //res.send(rows)
                obj = {info : rows, Error, err}
        ...
        if(err) throw err
            const params = req.body
                ...
                pool.getConnection((err, connection2) => {
                    connection2.query(`SELECT COUNT(id) AS count FROM info WHERE id = ${params.id}`, (err, rows) => {
                        if(!rows[0].count){
                            connection.query('INSERT INTO info SET ?', params, (err, rows) => {
                                connection.release()
                                if(!err){
...
app.post('/delete',(req, res) => {     //ใช้ POST เข้าจัดการจะปลอดภัยกว่าแบบเดิม
    ...
    pool.getConnection((err, connection) =>{
        ...
        connection.query('DELETE FROM `info` WHERE `info`.`id` = ?', [id], (err, rows) => {
            connection.release();
            if(!err){ 
    ...
    pool.getConnection((err, connection) =>{
        ...
        connection.query('SELECT * FROM info', (err, rows) => {
            ...
            if(!err){ 
                //--------Model of Data--------------//
                obj = { info : rows, Error : err, mesg : mesg}
                //-----------Controller--------------//
                res.render('deleteitem', obj)
...
app.put('/update',(req, res) => {
    pool.getConnection((err, connection) =>{
        ...
        //สร้างตัวแปรแบบกลุ่ม
        const {id, prize, description} = req.body       

        //Update ข้อมูลต่าง ๆ ตามลำดับ โดยใช้เงื่อนไข id
        connection.query('UPDATE info SET prize = ?, description = ?, WHERE id = ?', [prize, description, id], (err, rows) => {
            connection.release();
            if(!err){ 
                res.send(`${description} is complete update item. `) 
            } else {
                console.log(err)
            }
        })
